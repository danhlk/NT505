...
def parse_args():
		...
		parser.add_argument('--local_model', type=Path, default=Path("/home/containernet/KLTN/Pesidious/CSV/StackingRandomForest.pkl"), required=True
...
def main():
	...
	args = parse_args()
	local_model = pickle.load(open(args.local_model, 'rb'))
	# local_model = joblib.load(open(str(args.local_model), 'rb'))
	info(local_model.__class__.__name__)
	logging_setup("classifier_logs_" + str(date.today()) + local_model.__class__.__name__ + ".log", args.log)
	...
	if(args.f):
		...
		if(args.remote_model_config):
			...
			# local_model = pickle.load(open(os.path.join(module_path, args.local_model), 'rb'))
	...
	#info(f"Total Benign(s): {len(benign_list)}", extra={"markup":True})
	#list_benign_file = open(os.path.join('list_files', str(date.today()) + '_' +local_model.__class__.__name__ + '.txt'), 'w')
	#for file in benign_list:
		#list_benign_file.write(file + '\n')

	#list_benign_file.close()
