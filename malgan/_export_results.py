...
def _export_results(model: 'MalGAN', valid_loss: TensorOrFloat, test_loss: TensorOrFloat,
                    ...
                    y_mal_orig: TorchOrNumpy, y_prob_before: TorchOrNumpy, y_prob: TorchOrNumpy, y_actual_mal_gen: TorchOrNumpy, y_hat: np.ndarray, cyc_len: int) -> str:
    r"""
    ...
    with open(str(results_file), "a+") as f_out:
        header = ",".join(["time_completed,M,Z,batch_size,Number of Epochs,test_set_size,detector_type,activation",
                           ...
                           "auc_before,accuracy_before, precision_before, recall_before, f1_before, acc_after, recall_after",])
        if not exists:
            f_out.write(header)
        ...
        auc_before = roc_auc_score(y_actual, y_prob_before)
        results.append("%.8f" % auc_before)

        # Calculate the detection rate on unmodified malware
        results.append("%.8f" % accuracy_score(y_actual, y_mal_orig))
        results.append("%.8f" % precision_score(y_actual, y_mal_orig))
        results.append("%.8f" % recall_score(y_actual, y_mal_orig))
        results.append("%.8f" % f1_score(y_actual, y_mal_orig))

        results.append("%.8f" % accuracy_score(y_actual_mal_gen, y_hat))
        results.append("%.8f" % recall_score(y_actual_mal_gen, y_hat))
