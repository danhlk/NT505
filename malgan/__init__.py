...
class MalGAN(nn.Module):
    ...
    def _fit_blackbox(self, mal_train: Subset, ben_train: Subset) -> None:
        ...
        filename = 'models/' + str(self._bb.type.name) + '_' + str(date.today()) + '.h5'
        pickle.dump(self._bb, open(filename, 'wb'))
    ...
    def measure_and_export_results(self, cyc_len: int, adversarial_feature_vector_directory: str, output_filename: str) -> str:
        ...
        num_mal_test = 0
        m_prime_arr, bits_changed, m_list = [], [], []
        for m, _ in self._mal_data.test:
            # m_list.append(self._bb.predict(m.cpu()))
            m_list.append(m.cpu())
            if self._is_cuda:
                m = m.cuda()
        ...
        x_test = torch.cat(m_list + ben_test_arr)
        # logging.info(len(x_test))
        # logging.info(len(x_test[0]))
        y_pred = self._bb.predict(x_test)
        y_prob_before = self._bb._model.predict_proba(x_test)[:,1]
        x = torch.cat(m_prime_arr)
        y_actual = torch.cat((torch.full((num_mal_test,), MalGAN.Label.Malware.value),
                             torch.full((len(x_test) - num_mal_test,), MalGAN.Label.Benign.value)))

        y_actual_mal_gen = torch.full((num_mal_test,), MalGAN.Label.Malware.value)
        y_hat_post = self._bb.predict(x)
        if self._is_cuda:
            y_pred, y_hat_post, y_actual = y_pred.cpu(), y_hat_post.cpu(), y_actual.cpu()
        ...
        return _export_results(self, valid_loss, test_loss, avg_changed_bits, y_actual,
                               y_pred, y_prob_before, y_prob, y_actual_mal_gen, y_hat_post, cyc_len)
